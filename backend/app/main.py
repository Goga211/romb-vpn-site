...
from . import db
from .config import get_settings, validate_runtime_config
...
async def lifespan(app: FastAPI):
    # Безопасность конфигурации: в проде небезопасные значения роняют старт.
...
if _dist.is_dir():
    ...
    _index_file = _dist_root / "index.html"

    @app.get("/{full_path:path}")
    async def serve_spa(full_path: str):
        """Раздаёт собранный фронт с SPA-фолбэком. Реальные файлы (assets, шрифты,
        telegram-web-app.js) отдаём как есть; для остальных путей — index.html,
        чтобы прямой заход/обновление на десктоп-маршрутах (/pricing, /login, /app)
        не давал 404. /api, /health, /open обрабатываются роутерами выше."""
        if full_path.startswith("api/"):
            raise HTTPException(status_code=404, detail="Not Found")
        if full_path:
            candidate = (_dist_root / full_path).resolve()
            if candidate.is_file() and str(candidate).startswith(str(_dist_root)):
